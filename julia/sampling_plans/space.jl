...
# get XY
Xidx1 = find(x -> x in Xgls, X[:, 1])
Xidx2 = find(x -> x in Xexc, X[:, 1])
# display(X[XglsXY, :])
...
scatter(x=X[Xidx1, 1], y=X[Xidx1, 2], color="red")
xticks([])
yticks([])
...
subplot(1, 2, 2)
scatter(x=X[:, 1], y=X[:, 2], color="black")
scatter(x=X[Xidx2, 1], y=X[Xidx2, 2], color="red")
xticks([])
yticks([])
title("exchange algorithm")
xlabel("x1")
tight_layout()
show()
close()
